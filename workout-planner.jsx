...
function YouTubeModal({ url, title, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}} onClick={onClose}>
...
function Stopwatch({ startTime, onTick }) {
  ...
  useEffect(() => {
    const id = setInterval(() => { const s=Math.floor((Date.now()-startTime)/1000); setElapsed(s); onTick?.(s); }, 1000);
...
function LibExercises({ library, setLibrary, askConfirm, setView, editingExId, setEditingExId, quickPick=false, onQuickPick }) {
  const groups = {};
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:editingExId===ex.id?12:0}}>
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:"#666688",marginTop:2,display:"flex",gap:8}}>
                      ...
                      {isYT(ex.youtubeUrl)&&<span style={{color:"#ff5555"}}>▶ YouTube</span>}
                    </div>
                  </div>
...
function LibWorkouts({ library, setLibrary, askConfirm, setView, quickPick=false, onQuickPick }) {
...
export default function App() {
  ...
  function pauseWorkout() {
    setActiveWorkout(aw=>aw?{...aw,completedSets,startTime:startRef.current,elapsed:elapsedRef.current,paused:true}:aw);
    ...
    return (
      <Wrap>
        ...
        {libTab==="exercises"&&<LibExercises library={library} setLibrary={setLibrary} askConfirm={askConfirm} setView={setView} editingExId={editExId} setEditingExId={setEditExId}
          quickPick={libReturn==="clientEdit"} onQuickPick={ex=>{setQuickLibPick({type:"exercise",data:ex});setView("clientEdit");}}/>}
        {libTab==="workouts"&&<LibWorkouts library={library} setLibrary={setLibrary} askConfirm={askConfirm} setView={setView}
          quickPick={libReturn==="clientEdit"} onQuickPick={w=>{setQuickLibPick({type:"workout",data:w});setView("clientEdit");}}/>}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:16}}>
          {dayExs.map((ex,idx)=>(
            <div key={ex.id} className="card up" style={{animationDelay:`${idx*.05}s`}}>
              ...
              {isYT(ex.youtubeUrl)&&getYTThumb(ex.youtubeUrl)&&<div style={{marginBottom:8}}><img src={getYTThumb(ex.youtubeUrl)} style={{width:"100%",borderRadius:10,opacity:.85}} alt=""/></div>}
                  ...
                  <div key={ex.id} className="up" style={{borderRadius:16,overflow:"hidden",border:`1px solid ${cardBorder}`,background:"linear-gradient(145deg,#12121e,#1a1a2a)",animationDelay:`${idx*.05}s`,transition:"border-color .2s"}}>
                    ...
                    <div style={{display:"flex",alignItems:"stretch",minHeight:90}}>
                      ...
                      <div style={{width:100,flexShrink:0,position:"relative",cursor:isYT(ex.youtubeUrl)?"pointer":"default"}}
                        ...
                        {isYT(ex.youtubeUrl)&&<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.35)",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{width:32,height:32,borderRadius:"50%",background:"rgba(255,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>▶</div></div>}
                      </div>
